...
begin
    if not Initialized then begin
        ...
        BandwidthCheckBox.Checked     := Boolean(IniFile.ReadInteger(SectionData, KeyBandwidthLimit, 0));
        BandwidthEdit.Text            := IniFile.ReadString(SectionData, KeyBandwidth,   '50000');
        IniFile.Free;
...
begin
    ...
    IniFile.WriteInteger(SectionData,   KeyBandwidthLimit, Ord(BandwidthCheckBox.Checked));
    IniFile.WriteString(SectionData,    KeyBandwidth,   BandwidthEdit.Text);
    IniFile.Free;
end;
...
{$IFDEF UseBandwidthControl}
        if BandwidthCheckBox.Checked then begin
...
begin
    ...
    ProgressTimer.OnTimer(Self);     { A last progress update }
    SetButtonState(TRUE);
    if ErrCode <> 0 then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpTestForm.HttpCli1DocData(
    Sender : TObject;
    Buffer : Pointer;
    Len    : Integer);
begin
    Inc(FDocRcvdCount, Len);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpTestForm.ProgressTimerTimer(Sender: TObject);
begin
    if HttpCli1.ContentLength > 0 then
        ProgressLabel.Caption :=
            Format('%d bytes %4.0f%%',
                   [FDocRcvdCount,
                    (FDocRcvdCount / HttpCli1.ContentLength) * 100.0])
    else
        ProgressLabel.Caption := Format('%d bytes', [FDocRcvdCount]);
end;
