...
const
  WSocketVersion            = 520;
  CopyRight    : String     = ' TWSocket (c) 1996-2005 Francois Piette V5.20 ';
    ...
    while bMore do begin
        ...
        if Len <= 0 then begin
            ...
            if Count = 0 then
                ...
                then begin
{$IFDEF DEBUG_OUTPUT}
                    OutputDebugString(Name + ' Winsock.Send failed ' + IntToStr(LastError));
{$ENDIF}
...
{$IFDEF USE_SSL}
    ...
    SslCtxRefCounter   := TRefCounter.Create;
    {$IFDEF DEBUG_OUTPUT}
    LogFileName := IncludeTrailingPathDelimiter(ExtractFileDir(ParamStr(0))) +
                   'Debug_Out' + '_' +
...
{$IFDEF USE_SSL}
    if Assigned(SslCtxRefCounter) then
        FreeAndNil(SslCtxRefCounter);
