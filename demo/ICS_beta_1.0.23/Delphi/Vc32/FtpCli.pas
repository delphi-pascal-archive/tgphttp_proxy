...
const
  FtpCliVersion      = 2100;
  CopyRight : String = ' TFtpCli (c) 1996-2005 F. Piette V2.100 ';
...
type
  ...
  protected
    ...
    FSslFtpFlag         : Boolean;    { V2.100 set when TSslFtpCli initialised }
...
{   FDataSocket.OnDisplay        := FOnDisplay; } { Debugging only }
    FDataSocket.HSocket          := aSocket;
{$IFDEF USE_SSL}
    if FSslFtpFlag then begin    { V2.100 }
...
{$IFDEF NEVER_USE_SSL}
        ...
        if PeekMessage(Msg, FDataSocket.Handle, WM_ASYNCSELECT, WM_ASYNCSELECT, PM_NOREMOVE) then
            Windows.OutputDebugString('PeekMessage 2')
        else
...
begin
    ...
    if DWORD (FStopTime) > DWORD (FStartTime) then
